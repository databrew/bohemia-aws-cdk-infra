...
class AnalysisStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, env = None, cluster = None) -> None:
        ...
        #######################################
        # Step 3a: Placeholder Create Analysis Pipeline
        #######################################
        ...
        task_definition = ecs.FargateTaskDefinition(
            self,
            "pipeline-analysis-task-definition",
            execution_role=ecs_role,
            task_role=ecs_role,
            family='pipeline-analysis',
            memory_limit_mib= 16384,
            cpu=2048
        )

        # this is the dockerhub image that points to dockerhub
        dockerhub_image = f'databrewllc/pipeline-analysis:{docker_version}'

        # attach the container to the task definition
        container_definition = task_definition.add_container(
            "pipeline-analysis-container",
        ...
        # ento pipeline dump
        analysis_pipeline = tasks.EcsRunTask(    
            self, "AnalysisJob",
        ...
        # analysis parallels
        analysis_fail_trigger = sfn.Fail(self, "Notify Failure in analysis!")

        analysis_pipeline.add_catch(analysis_fail_trigger)

        parallel = (analysis_pipeline).next(success_trigger)
        ...
        if (os.getenv('PIPELINE_STAGE') == 'production'):
            ...
            midnight_schedule = events.Rule(
                self, "AnalysisMidnightSchedule",
                schedule=events.Schedule.expression("cron(00 21 * * ? *)"),
                targets=[targets.SfnStateMachine(state_machine)]
            )

        cdk.CfnOutput(self, 
                      'AnalysisStepFunction', 
                      value=state_machine.state_machine_arn,
                      export_name='analysis:arn')